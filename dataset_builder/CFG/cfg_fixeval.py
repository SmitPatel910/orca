...
from CFG.utils import get_block_ranges_and_statements, get_block_connections, renumber_cfg_blocks, generate_cfg_text, map_execution_order_to_blockwise
...
def process_single_code_submission(code, mode):
    try:
        ...
        return cfg_block_statements, cfg_block_range, cfg_block_connection
...
def process_dataset_entries(fixeval_dataset):
    ...
    for prob_id, problem_data in tqdm(fixeval_dataset.items()):
        ...
        for sub_id, submission in problem_data.items():
            ...
            cfg_block_statements, cfg_block_range, cfg_block_connection = cfg_items

            ground_truth_blocks = map_execution_order_to_blockwise(cfg_block_range, execution_order, exe_trace)

            cfg_text = generate_cfg_text(cfg_block_statements, cfg_block_connection, cfg_block_range)
            if cfg_text == "":    false_count += 1; continue

            results[prob_id][sub_id] = create_submission_entry(code, exception_info, execution_order, ground_truth_blocks, cfg_block_statements, cfg_block_connection, cfg_block_range, cfg_text)
            
            true_count += 1
    
    return results

def create_submission_entry(code, exception_info, execution_order, ground_truth_blocks, cfg_block_statements, cfg_block_connection, cfg_block_range, cfg_text):
    return {
        'code': code,
        ...
        'cfg_next_block': cfg_block_connection,
