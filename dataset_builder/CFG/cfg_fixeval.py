...
if __name__ == '__main__':
    input_prefers = input("Do you want to use CFG for particular program? (y/n): ")
    if input_prefers == 'y':
        from example import testFun
        graph = control_flow.get_control_flow_graph(testFun)
        source = program_utils.getsource(testFun)
        control_flow_graphviz.render(graph, include_src=source, path='cfg.png')
        with open('example.py', 'r') as file:
            code = file.read()
        cfg_block_statements, cfg_block_range, cfg_next_block = read_cfg(graph, code)
        f_cfg_block_statements, f_cfg_block_range, f_cfg_next_block = renumbering_blocks(cfg_block_statements, cfg_block_range, cfg_next_block)
        input_cfg = make_cfg_into_code(f_cfg_block_statements, f_cfg_next_block, f_cfg_block_range)
