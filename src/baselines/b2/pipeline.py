...
def gpt_pipeline(args, is_complete_code, code, ground_truth_execution_order, ground_truth_exception_info):
    '''Execute the GPT pipeline for baseline b2 evaluation.
    ...
    Returns:
        tuple: A tuple containing:
            ...
            - pred (dict): A dictionary containing:
                ...
                - "symbol_table" (dict): The symbol table generated during execution.
    ...
    try:
        ...
        # Output Parser
        pred_statement_exe, [pred_error_type, pred_is_error], pred_symbol_table = output_parser(output)
        ...
        pred = {
            ...
            "is_error": pred_is_error,
            "symbol_table": str(pred_symbol_table)
        }
...
def process_dataset(args, dataset, is_complete_code):
    '''Process the dataset using the GPT pipeline for baseline b2 evaluation.
    ...
    for probID in tqdm(dataset.keys()):
        ...
        baseline_b2[probID] = submissions
...
if __name__ == "__main__":
    ...
    complete_b2_res = process_dataset(args, complete_code_data, True)
    ...
    with open(complete_code_res_dir, 'w') as file:
        json.dump(complete_b2_res, file)
    ...
    incom_b2_res = process_dataset(args, incomplete_code_data, False)
    # Save the results
    print("Saving incomplete code results...")
    with open(incomplete_code_res_dir, 'w') as file:
        json.dump(incom_b2_res, file)

    print("\nBaseline B2 Pipeline Finished!\n")